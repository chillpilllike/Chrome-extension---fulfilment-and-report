...
export function AirwallexRefunds({stores,storeId,api}:Props) {
 ...
 useEffect(()=>{api<{connected:boolean}>('/api/airwallex/refunds/connection').then(r=>setConnected(r.connected)).catch(e=>setError(String(e)));void refreshHistory();void refreshLimits();const timer=setInterval(()=>{void refreshHistory();void refreshLimits()},15000);return()=>clearInterval(timer)},[])
 ...
 const canReview=!!daily&&daily.remaining>0&&!!snapshot&&!!schema&&!loadingSchema&&!busy&&recipientConfirmed&&otherChecked&&Number(amount)>0&&Number(amount)<=Number(snapshot.remaining)&&(!editing||!!editReason.trim())
