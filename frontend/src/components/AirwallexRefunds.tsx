...
type DailyLimit = {limit:number;used:number;remaining:number;timezone:string;date:string;resets_at:string}
...
export function AirwallexRefunds({stores,storeId,api}:Props) {
 ...
 const refreshFunding=()=>api<{currencies:FundingCurrency[]}>('/api/airwallex/refunds/funding-currencies').then(r=>setFunding(r.currencies)).catch(e=>{setFunding([]);setError(String(e))})
 const [daily,setDaily]=useState<DailyLimit|null>(null)
 const refreshLimits=()=>api<DailyLimit>('/api/airwallex/refunds/limits').then(setDaily).catch(e=>{setDaily(null);setError(String(e))})
 ...
 useEffect(()=>{api<{connected:boolean}>('/api/airwallex/refunds/connection').then(r=>setConnected(r.connected)).catch(e=>setError(String(e)));void refreshHistory();void refreshLimits();void refreshFunding();const timer=setInterval(()=>{void refreshHistory();void refreshLimits()},15000);return()=>clearInterval(timer)},[])
 ...
 const canReview=funding.some(f=>f.currency===sourceCurrency&&f.status==='available')&&!!daily&&daily.remaining>0&&!!snapshot&&!!schema&&!loadingSchema&&!busy&&recipientConfirmed&&otherChecked&&Number(amount)>0&&Number(amount)<=Number(snapshot.remaining)&&(!editing||!!editReason.trim())
 ...
 return <div className="space-y-5">
  ...
  <div role="status" className="rounded-lg border bg-muted/40 p-4 text-sm">{daily?<><strong>Daily refund limit: {daily.used} / {daily.limit} used · {daily.remaining} remaining</strong><p className="mt-1 text-muted-foreground">Shared across all stores and staff. Resets at midnight India time (Asia/Kolkata). Submitted attempts, including pending or uncertain refunds, count toward the limit. Airwallex transfers labelled “refund” are included.</p>{daily.remaining===0&&<p className="mt-2 font-medium text-red-700">Daily limit reached. New refunds are blocked until the next day.</p>}</>:"Checking the daily refund limit…"}</div>
