...
export function TeamWorkspaceHeader({ title, description, current, children }: {
  ...
  return <header className="team-workspace-header">
    ...
    <nav className="team-workspace-links" aria-label="Related operations pages">
      {[["orders", "1 · Orders"], ["package-pickups", "2 · Pickup check"], ["package-tracker", "3 · Package tracker"], ["after-order-care", "4 · After-order care"], ["epost", "ePost exceptions"], ["support", "Customer support"], ["email-log", "Email log"]].map(([path, label]) =>
