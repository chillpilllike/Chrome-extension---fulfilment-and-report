...
assert.match(app, /<Table className="epost-work-table care-work-table">/)
assert.match(app, /<DialogContent className="epost-detail care-review-dialog">/)
assert.match(app, /onClick=\{\(\) => setReviewCaseId\(row.id\)\}/)
assert.match(app, /filter\(row => row.id === reviewCaseId\)/)
assert.doesNotMatch(app, /className="care-case-summary"/)
console.log('PASS: ePost table and review dialog, queue rendering, search and queue callbacks, pagination wiring, and eight existing action handlers preserved')
