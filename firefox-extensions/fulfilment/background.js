...
async function clearFailedJobs() {
  ...
  await log(result.message || `Cleared ${result.cleared || 0} failed Firefox job line(s).`);
  return result;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === "GET_ACTIVE_JOB") {
      ...
      if (activeJob?.job && activeJob?.workerId && Date.now() - Number(activeJob.lastHeartbeatAt || 0) > 5 * 60 * 1000) {
        try {
          ...
          await log(`Firefox job lock heartbeat failed: ${error.message}`, windowId);
        }
      }
    ...
    if (message.type === "FAIL_JOB") return failJob(message.message || "Firefox extension job failed.", message, windowId);
    if (message.type === "COSTLY_JOB") return costlyJob(message.message || "Firefox extension job needs costly approval.", message, windowId);
