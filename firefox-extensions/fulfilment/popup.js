...
function renderQueuedJobs(jobs, workerId = "") {
  ...
  if (!queue.length) {
    ...
    empty.textContent = "No queued Firefox orders.";
  ...
  for (const job of queue) {
    ...
    const lockText = isLocked
      ? `${isMine ? "Locked by this Firefox" : "Locked by another Firefox"}${job.claim_expires_at ? ` until ${new Date(job.claim_expires_at).toLocaleTimeString()}` : ""}`
      : "Ready";
...
document.querySelector("#clearFailed").addEventListener("click", async () => {
  const confirmed = confirm("Clear failed or stale queued Firefox jobs in the app so they can be queued again?");
  if (!confirmed) return;
